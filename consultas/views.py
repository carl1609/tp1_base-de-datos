...
def evaluar_consultas(request, consulta):
    ...
    consultas = {
        ...
        "tablas-primarykey": "SELECT tc.constraint_name, tc.constraint_type, kcu.column_name FROM information_schema.table_constraints tc JOIN information_schema.key_column_usage kcu ON tc.constraint_name = kcu.constraint_name WHERE tc.constraint_type = 'PRIMARY KEY' AND tc.table_schema = 'public';",
        ...
        "privilegios-funciones": "SELECT grantee, privilege_type, is_grantable FROM information_schema.routine_privileges ORDER BY privilege_type",
...
def consulta_especifica (request, consulta):

    consultas = {
        "bbdd-tablas-de-esquema": "SELECT tablename FROM pg_tables WHERE schemaname = %s;",
        "privilegios-tabla": "SELECT grantee, privilege_type, is_grantable FROM information_schema.table_privileges WHERE table_name = %s;",
        "tamanio-tablas-esquema": "SELECT table_name, pg_size_pretty(pg_total_relation_size(quote_ident(table_schema) || '.' || quote_ident(table_name))) AS size FROM information_schema.tables WHERE table_schema = %s;",
        "tamanio-tabla": "SELECT pg_size_pretty(pg_total_relation_size(%s)) AS total_size"
    }

    def obtener_consulta(consulta, parametros):
        with connection.cursor() as cursor:
            print(consultas[consulta])
    ...
    if request.method == 'GET':
            return render (request, 'formConsulta.html', {'consulta':consulta})
    else:
        consulta = request.POST.get('consulta')
        if consulta == 'bbdd-tablas-de-esquema' or consulta == 'tamanio-tablas-esquema':
            resultado, nombre_columnas = obtener_consulta(consulta, request.POST.get('esquema'))
        else:
            resultado, nombre_columnas = obtener_consulta(consulta,request.POST.get('tabla'))
